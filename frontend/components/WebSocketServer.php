...
class WebSocketServer {
    ...
    public $handler;
    /**
     * Функция вызывается, когда клиент присоединился
     */
    public $enter_handler;
    /**
     * IP адрес сервера
     */
    private $ip;
    ...
    public function startServer() {
        ...
        while (true) {
            ...
            if (in_array($this->connection, $read)) {
                ...
                if (($connect = socket_accept($this->connection)) && $this->handshake($connect)) {
                    ...
                    $this->customers[] = array(
                      ...
                      'rooms' => [],
                      'user' => '',
                      'connect' => $connect, // добавляем его в список необходимых для обработки
                    );
                }
            ...
            foreach ($read as $connect) { // обрабатываем все соединения, в которых есть данные для чтения
                ...
                echo 'decoded: ';
                var_dump($decoded);
                $parsed = json_decode($decoded['payload'], true);
                echo 'parsed: ';
                var_dump($parsed);
                ...
                if (false === $decoded || 'close' === $decoded['type']) {
                    ...
                    if (!empty($this->customers)) {
                      foreach ($this->customers as $key => $customer) {
                        if (isset($parsed['id']) && $customer['id'] == $parsed['id']) {
                    ...
                    $this->debug('Connection closing 5');
                    echo 'customers 2: ';
                    var_dump($this->customers);
                    foreach ($this->customers as $item) {
                      socket_write($item['connect'], self::encode(json_encode(['type' => 'chatUserList', 'data' => $userList])));
                    }
                ...
                $user_added = false;
                if (isset($parsed['user']) && !empty($this->customers)) {
                  foreach ($this->customers as $key => $customer) {
                    if ($customer['connect'] == $connect) {
                      if (!$customer['user']) {
                        $this->customers[$key]['id'] = $parsed['id'];
                        $this->customers[$key]['user'] = $parsed['user'];
                        if ( isset($parsed['room_id']) && !in_array((int)$parsed['room_id'], $this->customers[$key]['rooms']) ) {
                          $this->customers[$key]['rooms'][] = (int)$parsed['room_id'];
                        }
                        $user_added = true;
                      }
                    }
                ...
                if ($user_added) {
                  ...
                  foreach ($this->customers as $item) {
                    self::response($item['connect'], json_encode(['type' => 'chatUserList', 'data' => $userList]));
                  }
                }
    ...
    private static function decode($data) {
        ...
        if ($opcode == 8) {
          $decodedData['payload'] = substr($decodedData['payload'], 2);
        }
        return $decodedData;
    }
