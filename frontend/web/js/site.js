            ...
            _createClass(Iti, [ {
                ...
                value: function _setFlag(countryCode) {
                    ...
                    if (this.options.separateDialCode) {
                        ...
                        this.telInput.style.paddingLeft = "".concat(selectedFlagWidth + 6 + 9, "px");
                    }
...
(function($) {
		...
		compactLabels: ['л', 'м', 'н', 'д', 'ч', 'м', 'с'], compactLabels1: ['г', 'м', 'н', 'д', 'ч', 'м', 'с'],
...
(function($) {
    ...
    if (window.chrome && !isMobileOrTablet()) {
    ...
    $(document).ready(function(){
		...
		//Preloader
		let isLoading = false,
			outputPercentage = document.querySelector('.preloader .percentage');
		if (outputPercentage) {
			if (!isLoading) {
				isLoading = true;

				let barLoad = document.querySelector(".load");
				let width = 1;
				...
				let startFrame = setInterval(function () {
					...
					outputPercentage.innerHTML = `${width}%`
				}, 10);
			}
		}
		...
		if ($('.bg-layer').length) {
			...
			const lineLayout = $('.bg-layer .line');

			$('.bg-layer').html('');

			let offsetPercent = 14;

			for (let i = 0; i <= 10; i++) {
				let newLine = lineLayout.clone();

				newLine.children('.animated-chunk').css('left', `${Math.floor(Math.random() * 60)}%`);

				if (i===6) offsetPercent = 18;

				if (i < 6) {
					newLine.css({
						'width': `${$('.bg-layer').width()}`,
						'top': `${offsetPercent}%`
					});
				} else {
					newLine.css({
						...
						'left': `${offsetPercent}%`,
						'width': `${$('.bg-layer').height()}`,
					});
				}

				$('.bg-layer').append(newLine);

				i < 6 ? offsetPercent += 15 : offsetPercent +=15;
			}
		}
        ...
        function setIncome() {
            ...
            let persent = parseInt($( "#amount1" ).val());
            let results = $('#payment-list1-res');
            let year_res = 0;
            results.find('li:first b i').text(number_format(Math.round(deposit/100*persent, 0), 0, '', ' '));
            for (var i2 = 1; i2 < 13; i2++ ) {
                year_res += (deposit + year_res)/100*persent;
            }
            results.find('li:last b i').text(number_format(Math.round(year_res, 0), 0, '', ' '));
        }
...
// табы
        ...
        jQuery(window).scroll(function(){
            ...
            $sections.each(function(i,el){
                ...
                if( scroll > top && scroll < bottom){
                    $('.main-header-menu a').removeClass('active');
                    $('a[href="#'+id+'"]').addClass('active');
                }
                if( scroll < $('#aboutWork').offset().top-100){
                    $('.main-header-menu a').removeClass('active');
                }
            })
        });
        ...
        $(window).scroll(function(){
            $('.main-header').toggleClass('active', $(this).scrollTop() > 0);
        });
        ...
        $(".faq-item").on('click', function(e) {
            e.preventDefault();
            if($(this).children('.open_toggle').next("div").is(":visible")){
                $(this).children('.open_toggle').next("div").slideUp(200);
                $(this).children('.open_toggle').removeClass("active");
            } else {
                $(".block_toggle").slideUp(200);
                $(this).children('.open_toggle').next("div").slideDown(200);
                $(this).children('.open_toggle').parents().siblings().children(".open_toggle").removeClass("active");
                $(this).children('.open_toggle').addClass("active");
            }
        });

        $('#popup1 .inactive').on('click', function() {
            $.fancybox.close();
            $.fancybox.open({
                src: '#popup2',
                touch : false
        ...
        $('#popup2 .inactive').on('click', function() {
            $.fancybox.close();
            $.fancybox.open({
                src: '#popup1',
                touch : false
        ...
        $('.main-header-menu a').on('click', function() {
...
//      --------------- Мой код идет дальше -----------------------------------------
        ...
        $(document).delegate('.grid-editable', 'change', function() {
            ...
            let input_attr = cur_input.attr('name').split('[')[1].split(']')[0];
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data['success'] === '1') {
                        ...
                        if (cur_input.next().hasClass('error-help')) {
                            cur_input.next().text('');
                        }
                    } else {
                        console.log(data);
                        if (data.value[input_attr]) {
                            if (cur_input.next().hasClass('error-help')) {
                                cur_input.next().text(data.value[input_attr]);
                            }
                        }
                    }
                },
		...
		$('header .main-header-tel').on('click', function() {
  		  ym(66152347,'reachGoal','clickPhone');
			  fbq('track', 'CompleteRegistration');
    });
		$('.main-header-soc-list a').on('click', function() {
			  ym(66152347,'reachGoal', $(this).attr('data-goal'));
			  fbq('track', 'CompleteRegistration');
    });

        $(document).delegate('#photoupload-picture', 'change', function(e) {
            $(this).closest('form').submit();
        });

        $(document).delegate('.accruals .accruals-select-container form select', 'change', function() {
            $('.accruals .accruals-select-container form [type="submit"]').trigger('click');
        });

        function setBotsTable(cur_tr) {
            let details = cur_tr.find('.promocode-details table tbody');
            let discount = parseFloat(cur_tr.find('input.discount-value').val());
            ...
            let html =  '<tr>' +
                        '<td>Пакет</td>' +
                        '<td>Без скидки, RUB</td>' +
                        '<td>Прибыль, RUB</td>' +
                        '<td>Скидка, RUB</td>' +
                        '<td>Со скидкой, RUB</td>' +
                        '</tr>';
            let cust_discount = 0;
            let bot_part_payback = 0;
            let bot_buyer_disc = 0;
            let bot_buyer_price = 0;
            for (let i in bots) {
                cust_discount       = Math.round(bots[i].price * cur_discount / 100, 0);
                bot_buyer_disc      = Math.round(cust_discount * discount / 100, 0);
                bot_buyer_price     = Math.round(bots[i].price - bot_buyer_disc, 0);
                bot_part_payback    = cust_discount - bot_buyer_disc;
                html += '<tr>' +
                        '<td>'+bots[i].name+'<br />$' +number_format(bots[i].deposit, 0, '', ' ')+'</td>' +
                        '<td>'+number_format(bots[i].price,         0, '', ' ')+'</td>' +
                        '<td>'+number_format(bot_part_payback,      0, '', ' ')+' ('+parseInt(cur_discount - (discount * cur_discount / 100))+'%)</td>' +
                        '<td>'+number_format(bot_buyer_disc,        0, '', ' ')+' ('+parseInt(discount * cur_discount / 100)+'%)</td>' +
                        '<td>'+number_format(bot_buyer_price,       0, '', ' ')+'</td>' +
                        '</tr>';
            }
            details.html(html);
        }

        $('.account-promocode-update').on('click', function() {
            let partner = !$(this).closest('.privateOfficePromotionalCodeSetting-table-wrap2').hasClass('single-row');
            let table = $(this).closest('table');
            let cur_tr = $(this).closest('tr');
            let old_tr = 0;
            let cur_slider = cur_tr.find('.slider-range-discount');
            let percents = cur_slider.closest('.privateOfficePromotionalCodeSetting-slider').find('.privateOfficePromotionalCodeSetting-slider-wrap1');
            let details_tbl = cur_tr.find('.promocode-details');
            if (cur_tr.hasClass('active')) {
                cur_tr.removeClass('active');
                cur_tr.children('td').css('padding-bottom', '7px');
                if (partner) {
                    cur_tr.children('td').find('.input').addClass('input-readonly').attr('readonly', 'readonly');
                }
                cur_tr.find('.slider-range-discount').slider('destroy');
            } else {
                old_tr = table.find('tr.active');
                if (old_tr.length) {
                    old_tr.children('td').css('padding-bottom', '7px');
                    if (partner) {
                        old_tr.children('td').find('.input').addClass('input-readonly').attr('readonly', 'readonly');
                    }
                    old_tr.find('.slider-range-discount').slider('destroy');
                    old_tr.removeClass('active');
                }

                cur_tr.addClass('active');
                cur_tr.children('td').css('padding-bottom', details_tbl.outerHeight()+'px');
                if (partner) {
                    cur_tr.children('td').find('.input').removeClass('input-readonly').removeAttr('readonly');
                }

                cur_slider.slider({
                    range: "max",
                    min: 0,
                    max: parseInt(cur_discount),
                    value: parseInt(parseInt(cur_tr.find('input.discount-value').val()) * parseInt(cur_discount) / 100),
                    change: function( event, ui ) {
                        cur_tr.find('input.discount-value').val(ui.value / parseInt(cur_discount) * 100).trigger('change');
                    },
                    slide: function( event, ui ) {
                        percents.find('i').text((parseInt(cur_discount) - ui.value)+'%');
                        percents.find('b').text(ui.value +'%');
                        cur_tr.find('input.discount-value').val(ui.value / parseInt(cur_discount) * 100);
                        cur_tr.find('.discount-title').text(ui.value+'%');
                        cur_tr.find('.yourincome-title').text((parseInt(cur_discount) - ui.value) +'%');
                        setBotsTable(cur_tr);
                    },
                });
                setBotsTable(cur_tr);
        ...
        $(document).delegate('input.input', 'keyup', function(e) {
            let form = 0;
            let btn = 0;
            if (e.keyCode == 13) {
                form = $(this).closest('form');
                if (form.length) {
                    btn = form.find('button.sitelogin-button');
                    btn.trigger('click');
                }
            }
        ...
        $(document).delegate('.sitelogin-button', 'click', function() {
			console.log('начало: ')
            ...
            if (action == 'register' || action == 'testrequest' || action == 'profileedit' || action == 'consultrequest' || action == 'payoutrequest') {
				
				if (action == 'testrequest') {
					//ym(66152347,'reachGoal','submitTry');
				} else if (action == 'consultrequest') {
					//ym(66152347,'reachGoal','submitTry');
				} else if (action == 'payoutrequest') {
					//ym(66152347,'reachGoal','submitPayout');
				}
				
				console.log('перед рекапчей: ')
                grecaptcha.execute('6Ld8b-kaAAAAAKKyKxG3w3RW0hCxqBelwko_jTFZ', {action: 'request_signup'})
                    .then(function (token) {

                        data.append('action', 'request_signup');
                        data.append('token', token);
                        data1 = form.serialize();
                        data1 += '&action=request_signup';
                        data1 += '&token='+token;
...
//                            contentType: false,
                            ...
                            success: function (json) {
                                ...
                                if (json['success'] === true) {
                                    if (action == 'register' ) {
                                      ...
                                      const expires = new Date(currentTime + 20 * 60000);
                                      const value = json['token'];
                                      localStorage.setItem('user_token', JSON.stringify({ value, expires }));
                                      location = '/lk/';
                                    } else if (action == 'payoutrequest') {
                                        location.reload();
                                    } else if (action == 'consultrequest') {										
    ...
    $('.customer-tree .open-levels').on('click', function () {
        let container = $(this).closest('.customer-tree').find('.referals-container');
    ...
    $('.customer-tree .close-levels').on('click', function () {
        let container = $(this).closest('.customer-tree').find('.referals-container');
        $(container.find('.open-childs.fa-minus-square-o:visible').get().reverse()).each(function () {
            $(this).trigger('quickclick');
        })
    });

    $(document).delegate('.customer-tree .referals-container .childs', 'expanded', function(e) {
        e.stopPropagation();
        console.log($(this));
        $(this).find('.open-childs.fa-plus-square-o').each(function () {
            $(this).trigger('quickclick');
        })
    });

    $(document).delegate('.client-area .open-childs', 'click quickclick', function(e) {
        let etype = e.type;
        let plus = $(this);
    ...
    $('.main-header-privateOffice .main-header-list li:nth-of-type(2) .main-header-list-info b').on('click', function() {

        let is_disabled = $(this).parent().hasClass('disabled');
        if (!is_disabled) {
            let text = $(this).text();
            Clipboard.copy('https://copy-trade.ru?ref=' + text);
            swal('Ваша персональная реф-ссылка скопирована в буфер обмена');
        } else {
            swal('Функция заблокирована!', "Система промокодов позволяет делиться скидками и зарабатывать.\n\n Промокоды будут доступны после первой покупки.");
        }

    });
	$(document).delegate('.copy-to-clip', 'click', function() {
    
		let text = $(this).text();
		console.log('text: ', text);
		Clipboard.copy(text);
		swal('Кошелек скопирован в буфер обмена');

    });

    $(document).delegate('#form-payoutrequest input[name="PayoutRequestForm[request_type]"]', 'change', function() {
        let form = $('#form-payoutrequest');
        let type = $('#form-payoutrequest input[name="PayoutRequestForm[request_type]"]:checked').val();
        if (type == 1) {
            form.find('.field-payoutrequestform-bank').removeClass('hidden');
            form.find('.field-payoutrequestform-receiver').removeClass('hidden');
            $('#payoutrequestform-card_number').attr('placeholder', 'Номер карты*');
        } else {
            form.find('.field-payoutrequestform-bank').addClass('hidden').find('input').val('');
            form.find('.field-payoutrequestform-receiver').addClass('hidden').find('input').val('');
            $('#payoutrequestform-card_number').attr('placeholder', 'Адрес криптокошелька*');
        }
