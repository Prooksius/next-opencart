...
class BlogPost extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['category_id', 'click', 'likes', 'user_id', 'status'], 'integer'],
            [['brief', 'content'], 'string'],
            [['banner'], 'string', 'max' => 255],
            [['title', 'tags', 'slug'], 'string', 'max' => 128],
            [['click', 'likes'], 'default', 'value' => 0]
        ];
    }
    ...
    public function getCommentsCount()
    {
        return BlogComment::find()
            ->andWhere(['post_id' => $this->id, 'status' => IActiveStatus::STATUS_ACTIVE])
            ->count();
    }
    ...
    public function getPublisher()
    {
        if ($this->getModule()->userModel) {
            return $this->hasOne($this->getModule()->userModel::className(), [$this->getModule()->userPK => 'user_id'])->one();
        }
        return null;
    }
    ...
    {
        if ($this->getModule()->userModel) {
            return $this->hasOne($this->getModule()->userModel::className(), [$this->getModule()->userPK => 'user_id'])->one()->{$this->getModule()->userName};
        }
        return '';
    }
    ...
    public function getTagLinks()
    {
        ...
        foreach (BlogTag::string2array($this->tags) as $tag) {
            $links[] = Html::a(Html::tag('span', $tag, ['class' => 'tag-item']), Yii::$app->getUrlManager()->createUrl(['blog/default/index', 'tag' => $tag]));
        }
