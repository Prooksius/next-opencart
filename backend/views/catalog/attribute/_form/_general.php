...
<div class="row">
    <div class="col-md-3 col-sm-4">
        <div class="picture-div">
            <label class="control-label"><?= $model->getAttributeLabel('icon') ?></label>
            <?= InputFileWithPic::widget([
                ...
                'name'       => StringHelper::basename(get_class($model)) . '[icon]',
                'value'      => $model->icon,
            ]); ?>
        </div>
    </div>
    <div class="col-md-9 col-sm-8">
        <div class="row">
            <div class="col-sm-6">
              <?= $form->field($model, 'attribute_group_id')->dropDownList($allGroups) ?>
              <?= $form->field($model, 'alias')->textInput(['maxlength' => true]) ?>
              <?= $form->field($model, 'value_type')->dropDownList($model->valueTypesList) ?>
              <?= $form->field($model, 'value_sort')->dropDownList($model->valueSortList) ?>
              <?= $form->field($model, 'sort_order')->textInput(['type' => 'number']); ?>
              <?= $form->field($model, 'filter_sort_order')->textInput(['type' => 'number']); ?>
            </div>
            <div class="col-sm-6">
              <?= $form->field($model, 'show_filter')->checkbox(); ?>
              <?= $form->field($model, 'show_product')->checkbox(); ?>
              <?= $form->field($model, 'show_product_tile')->checkbox(); ?>
